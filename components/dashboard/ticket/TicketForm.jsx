...
function TicketForm({ openOverlay, setOpenOverlay }) {
  const {
    ...
    control,
    handleSubmit,
  } = useTicket({ openOverlay, setOpenOverlay });
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="grid grid-rows-[1fr_auto] h-full w-full">
      <LoaderWarpper isLoading={isLoading}>
              ...
              render={({ field, fieldState: { error } }) => {
                return (
                  <TextAreaField
                    ...
                    readOnly={openOverlay === BUTTON.VIEW}
                    label="closedReason"
                    placeholder="Enter closedReason..."
                    error={error}
                  />
                )
              }}
        ...
        <div className="flex justify-end flex-col gap-2 sm:flex-row border-t py-2 w-full">
          {openOverlay === BUTTON.CLOSE && <Button text={"Close Ticket"} type="submit" isLoading={loaderHandler(isLoading)} disabled={loaderHandler(isLoading)} className="w-full" />}
        </div>
        </LoaderWarpper>
    </form>
  );
