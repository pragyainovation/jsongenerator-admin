...
function EditPassword({ user }) {
  const { handleSubmit, control, isLoading, onSubmit, openDialog, setOpenDialog } = useUpdatePassword({ user });
  return (
      ...
      <LoaderWarpper isLoading={isLoading}>
        <form onSubmit={handleSubmit(onSubmit)} className="w-full h-full grid grid-rows-[1fr_auto] gap-2">
          <div className="border rounded h-full p-2">
            <HookFormInput name="oldPassword" label="Old Password" className="!mb-2" control={control} placeholder="Enter your old password" />

            <HookFormInput name="newPassword" label="New Password" control={control} placeholder="Enter your new passsword" />
          </div>
