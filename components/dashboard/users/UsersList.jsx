...
function UsersList() {
  const { data, isLoading, columns, onPageChange, searchListData, handleConfirm, openDialog, setOpenDialog, openOverlay, setOpenOverlay } = useUsersList();
  ...
  return (
    ...
    >
      ...
      <DialogBox isOpen={openDialog} onClose={() => (loaderHandler(isLoading) ? {} : setOpenDialog(null))}>
        {openDialog === BUTTON.EDIT && <ConfirmationBox openDialog={openDialog} isLoading={isLoading} itemName={data?.userData?.fullName} buttonClick={() => (openDialog ? handleConfirm(openDialog) : undefined)} />}
      </DialogBox>
