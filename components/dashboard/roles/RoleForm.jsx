...
function RoleForm({ openOverlay, setOpenOverlay }) {
  const { data, isLoading, permissionLoading, permissionDetails, onSubmit, handleSubmit, handleCheck } = useRoles({ openOverlay, setOpenOverlay });
      ...
      <LoaderWarpper isLoading={isLoading || permissionLoading}>
        <div className="overflow-y-auto custom-scrollbar h-full w-full flex flex-col gap-2 p-1">
          {permissionDetails?.permissionList &&
            Object.entries(permissionDetails?.permissionList).map(([key, value]) => {
              return (
                <div key={key}>
                  <label>{key}</label>
