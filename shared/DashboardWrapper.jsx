    ...
    <div className="relative w-screen h-screen overflow-hidden flex">
      {isLoading && (
        <div className="absolute top-0 bottom-0 left-0 right-0 flex justify-center items-center bg-white bg-opacity-50 z-10">
          <SimpleLoader text={'loading...'} className="flex gap-2 items-center" />
        </div>
      )}
      ...
      >
        ...
        <div className="w-full h-full flex flex-col gap-2">
          <div className="h-10 border-b pb-2 border-b-gray4 flex items-center justify-between">
            ...
            <Button className="sm:hidden" onClick={() => setMobileOpen(false)} noClass icon={<CloseIcon />} />
          </div>
          <div className="flex flex-col gap-1 w-full h-full overflow-y-auto scrollbar-hide">
            <Sidebar />
          </div>
          <div className="flex flex-col gap-2">
            <Button text={'Profile'} className={'w-full italic'} onClick={handleProfile} />
            <Button text={'Logout'} className={'w-full italic'} onClick={handleLogout} />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-rows-[auto_1fr] w-full h-full p-2 overflow-hidden">
        ...
        <div className="h-10 border-b border-b-gray4 flex w-full bg-white items-center justify-between">
          <Button className="hidden sm:block" onClick={() => setOpen(!isOpen)} noClass icon={<MenuBarIcon />} />
          <Button
            className="sm:hidden"
            onClick={() => {
        ...
        {/* Main content */}
        <div className="box-border h-full w-full overflow-hidden p-2">{children}</div>
      </div>
    </div>
  );
}
