...
function Button({ id, tooltip, text = null, onClick, type = 'button', className = '', disabled = false, onlyOutLine = false, icon = null, isLoading = false, noClass = false, ...other }) {
  ...
  const buttonClass = classNames(className, 'capitalize', {
    ...
    'bg-transparent hover:bg-secondary !border-secondary text-white': onlyOutLine && !isDark && !disabled && !noClass,
    'bg-secondary hover:bg-primary text-white border-white text-white': !onlyOutLine && isDark && !disabled && !noClass,
    'bg-secondary hover:bg-primary text-white text-black border-black': !onlyOutLine && !isDark && !disabled && !noClass,
    'cursor-not-allowed bg-light hover:bg-light !text-secondary border-secondary': disabled && !noClass,
  });
